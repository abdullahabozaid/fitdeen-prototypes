...
export default function Coach() {
  ...
  return (
    <div className="pad coach-pad">
      <div className="top">
        <div className="eyebrow">AI coach</div>
      </div>
      <div className="greet coach-title"><span className="coach-mark"><IconSparkle size={20} /></span>Coach</div>

      <div className="coach-feed" ref={feed}>
        {msgs.map((m, i) =>
          m.role === 'coach' ? (
            <div className="coach-line coach-from" key={i}>
              <span className="coach-bmark"><IconSparkle size={13} /></span>
              <div className="coach-bubble coach-cb">{m.text}</div>
            </div>
          ) : (
            <div className="coach-line coach-mine" key={i}>
              <div className="coach-bubble coach-ub">{m.text}</div>
            </div>
          )
        )}
        {typing && (
          <div className="coach-line coach-from">
            <span className="coach-bmark"><IconSparkle size={13} /></span>
            <div className="coach-bubble coach-cb coach-typing"><i /><i /><i /></div>
          </div>
        )}
      </div>

      {msgs.length <= coachSeed.length && (
        <div className="coach-chips dA-stagger">
          {coachPrompts.map((p) => (
            <button className="coach-chip" key={p} onClick={() => send(p)}>{p}</button>
          ))}
        </div>
      )}
      ...
      <div className="coach-input">
        ...
        />
        <button className="coach-send" onClick={() => send(draft)} aria-label="Send"><IconSend /></button>
      </div>
    </div>
  )
}
